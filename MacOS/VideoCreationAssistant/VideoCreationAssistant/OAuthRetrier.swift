...
class OAuthRetrier: RequestRetrier, RequestAdapter {
  ...
  public func should(_ manager: SessionManager, retry request: Request, with error: Error, completion: @escaping RequestRetryCompletion) {
    print("should")
    if let response = request.task?.response as? HTTPURLResponse, 403 == response.statusCode, 401 == response.statusCode, let req = request.request {
      print("there")
      print(">>>>>>>>")
      print(response)
